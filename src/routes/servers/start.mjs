...
export default async (app, minecraftServerRoot) => {
  app.post("/server/:server/start", async (req, res) => {
    ...
    await new Promise((resolve, reject) => {
      exec(`${path.join(serverPath, "start.sh")}`, { shell: "/bin/bash" }, (error, stdout, stderr) => {
        console.log(`Starting ${req.params.server}...`);
        if (error) {
    ...
    console.log(`Done starting ${req.params.server}.`);
